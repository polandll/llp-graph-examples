...
//schema.propertyKey('country').Text().properties('livedIn').ifNotExists().create()
...
// Search
//schema.vertexLabel('recipe').index('search').search().by('instructions').asText().by('name').asString().ifNotExists().add()
//schema.vertexLabel('FridgeSensor').index('search').search().by('location').ifNotExists().add()
//schema.vertexLabel('author').index('search').search().by('nickname').by('name').asString().ifNotExists().add()
...
g.V().hasLabel('recipe').has('instructions', Search.tokenPrefix('Sea')).values('name')
// expected answer: Oysters Rockefeller [seasonings], Roast Pork Loin [Season]
g.V().hasLabel('recipe').has('instructions', Search.tokenRegex('.*sea.*in.*')).values('name')
