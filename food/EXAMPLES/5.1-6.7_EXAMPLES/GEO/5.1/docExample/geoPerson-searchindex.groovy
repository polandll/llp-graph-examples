// Geodetic Person example - SEARCH INDEX

system.graph('geoPersonSearch51').create()
:remote config alias g geoPersonSearch51.g
...
// SCHEMA FOR PERSON AT LOCATION
schema.vertexLabel('person').properties('name','location').create()
...
//Create people at locations
graph.addVertex(label, 'person', 'name', 'Jamie Oliver', 'location', 'London')
graph.addVertex(label, 'person', 'name', 'Julia Child', 'location', 'Paris')
...
// Test finding a person with geo info
g.V().hasLabel('person').has('point', Geo.inside(Geo.polygon(-6.26031, 53.349805, 6.083887, 50.775346, 139.691706, 35.689487))).values('name')
// Test point
