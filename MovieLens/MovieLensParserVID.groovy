...
import com.datastax.bdp.graph.api.schema.IdPropertyKey
    ...
    public static List parse(final Graph graph, final GraphTraversalSource g, final String dataDirectory, final int maxRatings, final int batchSize) {
        ...
        new File(dataDirectory + '/movies.dat').eachLine { final String line ->
            ...
            genres.split('\\|').each { def genre ->
                def genreVertex = g.V(["~label":"genre", "name": genre]).tryNext().orElseGet {graph.addVertex(label, "genre", "name", genre)}
        ...
        new File(dataDirectory + '/users.dat').eachLine { final String line ->
            ...
            def occupationVertex = g.V(["~label":"occupation", "jobId": jobId]).tryNext().orElseGet {graph.addVertex(label, "occupation", "jobId", jobId, "name", occupations.g
et(jobId))}
        ...
        new File(dataDirectory + '/ratings.dat').eachLine { final String line ->
            ...
            def userTraversal = g.V(["~label":"person", "userId": userId])
            def movieTraversal = g.V(["~label":"movie", "movieId": movieId])
    ...
    public static void createSchema(final DseGraph graph) {
        graph.migration("setup", { def schema ->
            ...
            def movie = schema.buildVertexLabel("movie").idComponent("movieId", IdPropertyKey.Type.Partition).add()
            def person = schema.buildVertexLabel("person").idComponent("userId", IdPropertyKey.Type.Partition).add()
            def genre = schema.buildVertexLabel("genre").idComponent("name", IdPropertyKey.Type.Partition).add()
            def occupation = schema.buildVertexLabel("occupation").idComponent("jobId", IdPropertyKey.Type.Partition).add()
